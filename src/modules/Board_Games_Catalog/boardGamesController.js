...
boardGamesController.addBoardGame = (req, res) => {
  const {name,minplayers,maxplayers,duaration,dateAdd,currentStatus} = req.body;
...
boardGamesController.updateBoardGame = (req, res) => {
  const IDgame = req.params.IDgame;
  const {new_gameID,new_name,new_minplayers,new_maxplayers,new_duaration,new_dateAdd,new_currentStatus} = req.body;
